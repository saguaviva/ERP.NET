...
#Region "CAMPOS"
    ...
    Public Property MODCOL() As String
        Get
            If PA = -1 Then Exit Property
            Try
                mMODCOL = general.nz(dvForm(pa).Row("MODCOL"), "")
            Catch ex As Exception : End Try
            Return general.nz(mMODCOL, "")
        End Get
        Set(ByVal Value As String)
            If PA = -1 Then Exit Property
            If general.nz(Value, "") <> general.nz(MODCOL, "") Then
                mMODCOL = general.nz(Value, "")
                dvForm(pa).Row("MODCOL") = general.nz(mMODCOL, "") : GuardarDV()
            End If
        End Set
    End Property
    ...
    Public Property COLTITULO() As String
        Get
            If PA = -1 Then Exit Property
            Try
                mCOLTITULO = general.nz(dvForm(pa).Row("COLTITULO"), "")
            Catch ex As Exception : End Try
            Return general.nz(mCOLTITULO, "")
        End Get
        Set(ByVal Value As String)
            If PA = -1 Then Exit Property
            If general.nz(Value, "") <> general.nz(COLTITULO, "") Then
                mCOLTITULO = general.nz(Value, "")
                dvForm(pa).Row("COLTITULO") = general.nz(mCOLTITULO, "") : GuardarDV()
            End If
        End Set
    End Property
        ...
        Try
            For i = 0 To modelo.detalleEscandallo.dvForm.Count - 1
                If i = 0 Then
                    s = s & " AND ("
                End If
                s = s & " FIL = """ & general.NS(modelo.detalleEscandallo.dvForm(i).Item("TEIXIT")) & """ OR "
            Next
            If i = 0 Then
                Return ""
            Else
...
'                        aColoresPiezas.Add(GENERAL.nz(dvForm(i).Item("COLTITULO"), ""))
'                    End If
'                End If
'            Next
...
'                    Try
'                        dr("COLTITULO") =general.nz(aColoresPiezas(i), "")
'                    Catch ex As Exception
