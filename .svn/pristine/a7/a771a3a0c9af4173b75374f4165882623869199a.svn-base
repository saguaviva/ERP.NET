...
#Region "VARIABLES"
    ...
    Public dtComisiones As DataTable
    Public comisiones_TotalBase As Double
    Public comisiones_TotalComision As Double
...
#Region "INICIAR"
    ...
    Public Sub New(ByVal tabla As DataTable, _
        ...
        Try
            sqlSinWhere = "SELECT repres.*, " & _
                " filiales.DESCRI AS NOMCENTRO " & _
                " FROM repres " & _
                " LEFT JOIN filiales ON (filiales.CODI = repres.CENTRO) "
            Try
                UltimoCodigo = CargaParametro("representante")
                ...
                If numeroRegistroActual = -1 Then
                    ...
                    sqlSel = sqlSinWhere & _
                            " WHERE REPRES.CENTRO = """ & centro & """ ORDER BY REPRES.CODI " & _
                            " LIMIT 1"
                Else
                    sqlSel = sqlSinWhere & _
                              " WHERE REPRES.CENTRO = """ & centro & """ AND REPRES.CODI = """ & UltimoCodigo & """ ORDER BY REPRES.CODI " & _
                              " LIMIT 1"
                End If
    ...
    Public Overrides Sub CargarIdentificadores()
        Try
            CargaTabla(tablaRepresentantes, CCRepres, CNRepres, dtIdentificadores, True)
            dtIdentificadores.Columns(CCRepres).Caption = rm.GetString("CODIGO")
            dtIdentificadores.Columns(CNRepres).Caption = rm.GetString("NOM")
            dvIdentificadores.RowFilter = "CENTRO = '" & centro & "' "
            dvIdentificadores.Sort = CNRepres
...
#Region "COMISIONES"
    ...
    Public Sub generarComisiones(ByVal d1 As Date, ByVal d2 As Date, ByVal iva As Double, ByVal irpf As Double)
        Dim strComisiones As String
        Dim dr As DataRow
        Try
            strComisiones = " SELECT " & _
                " FRA, DATA, " & _
                " CLIENTS.NOM AS NOMCLIENT, " & _
                " BASE1, " & _
                " COMIS, " & _
                " ROUND(COMIS/100*BASE1,2) AS TOTALCOMIS " & _
                " FROM FACTUR " & _
                " INNER JOIN CLIENTS ON (CLIENTS.CODI = FACTUR.CLIENT) " & _
                " WHERE " & _
                " FACTUR.REPRES = """ & CODI & """ AND " & _
                " FACTUR.DOCUMENT = """ & Factura & """ AND " & _
                " FACTUR.DATA >= """ & ConvertirAfechaMysql(d1) & """ AND " & _
                " FACTUR.DATA <= """ & ConvertirAfechaMysql(d2) & """ AND " & _
                " FACTUR.CENTRO = """ & empresaPorDefecto & """ "

            Dim da As New MySqlDataAdapter(strComisiones, cnn)
            dtComisiones = New DataTable '(New MySqlCommand(strComisiones, cnn))
            ACN()
