...
#Region "VARIABLES"
    ...
    Private dtRepres As New DataTable
        ...
        Try
            Select Case tabla
                Case "factur"
                    ...
                    CargaTabla(tablaRepresentantes, CCRepres, CNRepres, dtRepreS)
                Case "cactur"
            ...
            For i = 0 To dv.Count - 1
                Select Case tabla
                    Case "factur"
                        ...
                        dv(i).Item("NOMREPRES") = general.OBN(dv(i).Item("REPRES"), dtRepres)
                    Case "cactur"
    ...
    Friend Overrides Sub CrearComandos()
        Try
            Select Case tabla.ToUpper
                Case "FACTUR"
                    'Facturas
                    cmdSelect.CommandText = "SELECT FRA, CLIENT, DATA, IVA1, BASE1, BRUT1, TOTAL, TRASPAS, DOCUMENT, tipus, REPRES FROM " & tabla & " " &
                        " WHERE DOCUMENT = """ & doc & """ AND " &
                        " CENTRO = """ & centro & """ "
                Case "CACTUR"
                    cmdSelect.CommandText = "SELECT FRA, PROVE, DATA, IVA1, BASE1, BRUT1, TOTAL FROM " & tabla & " " &
                        " WHERE DOCUMENT = """ & doc & """ AND " &
                        " TIPUS = """ & tipo & """ AND " &
                        " CENTRO = """ & centro & """ "
                Case "ORDRE"
                    'Orden fab modelos
                    cmdSelect.CommandText = "SELECT ORDRE, CLIENT, DATA FROM " & tabla & " " &
                                                " CENTRO = """ & centro & """ "
            End Select
