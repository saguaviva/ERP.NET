...
#Region "CAMPOS"
    ...
    Public Property FET() As String
        Get
            If PA = -1 Then Exit Property
            Try
                mFET = general.nz(dvForm(pa).Row("FET"), "")
            Catch ex As Exception : End Try
            Return general.nz(mFET, "")
        End Get
        Set(ByVal Value As String)
            If PA = -1 Then Exit Property
            mFET = general.nz(Value, "")
            dvForm(pa).Row("FET") = general.nz(Value, "") : guardarDV()
        End Set
    End Property
        ...
        Set(ByVal Value As Integer)
            ...
            If esCodigoExistente(dtOperaciones, "CODI", Value) Then
                If nzn(Value, 0) <> 0 Then
                    mCODI = nzn(Value, 0)
                    dvForm(PA).Row("DESCRI") = general.OBN(Value, dtOperaciones, "DESCRI")
