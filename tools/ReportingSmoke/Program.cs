...
using Erp.Infrastructure.MySql.Reporting;
using Microsoft.Extensions.Options;
using MySqlConnector;

var exitCode = await ReportingSmokeRunner.RunAsync(args);
Environment.ExitCode = exitCode;

internal static class ReportingSmokeRunner
{
    ...
    public static async Task<int> RunAsync(string[] args)
    {
        ...
        {
            Console.WriteLine("No hay empresas activas para probar reporting.");
            return 0;
        }
        ...
        foreach (var company in companies)
        {
            ...
            var reporting = CreateReportingService(connectionFactory, company);

            failures.AddRange(await RunCaseAsync(
                company,
                "overview",
                () => reporting.GetOverviewAsync(company.TenantId, company.CompanyId)));

            failures.AddRange(await RunCaseAsync(
                company,
                "documents:all:no-date",
                () => reporting.SearchOperationalDocumentsAsync(
                    company.TenantId,
                    company.CompanyId,
            ...
            foreach (var category in new[] { "sales", "purchases", "production", "warehouse" })
            {
                var localCategory = category;
                failures.AddRange(await RunCaseAsync(
                    company,
                    $"documents:{localCategory}:no-date",
                    () => reporting.SearchOperationalDocumentsAsync(
                        company.TenantId,
                        company.CompanyId,
                        new OperationalDocumentFilter
                ...
                failures.AddRange(await RunCaseAsync(
                    company,
                    $"documents:{localCategory}:last30d",
                    () => reporting.SearchOperationalDocumentsAsync(
                        company.TenantId,
                        company.CompanyId,
                        new OperationalDocumentFilter
            ...
            failures.AddRange(await RunCaseAsync(
                company,
                "stats:last30d",
                () => reporting.GetBusinessStatisticsAsync(
                    company.TenantId,
                    company.CompanyId,
        ...
        if (failures.Count == 0)
        {
            Console.WriteLine("Reporting smoke: OK");
            return 0;
        }

        Console.Error.WriteLine();
        Console.Error.WriteLine("Reporting smoke: FAIL");
    ...
    private static MySqlReportingService CreateReportingService(
        ...
        CompanyProbeDto company)
    {
        ...
        return new MySqlReportingService(
            connectionFactory,
            companyAccess,
            currentUser,
            tenantContext,
    ...
    private sealed class FakeCurrentUserContext : ICurrentUserContext
    {
        ...
        public string Email => "reporting-smoke@local";
        public string DisplayName => "Reporting Smoke";
